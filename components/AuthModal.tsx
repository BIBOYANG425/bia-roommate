...
export default function AuthModal({
  ...
  const displayTitle = title || (mode === 'signup' ? 'CREATE ACCOUNT' : 'SIGN IN')
  const displaySubtitle = subtitle || (mode === 'signup'
        ...
        {success && mode === 'signup' ? (
          ...
          <div className="text-center py-4">
            ...
            <p className="text-sm mb-4" style={{ color: 'var(--mid)' }}>
              You&apos;re now signed in as <strong style={{ color: 'var(--black)' }}>{email}</strong>
            </p>
            <button
              onClick={handleClose}
              className="px-6 py-2 font-display text-sm tracking-wider text-white border-[3px] border-[var(--black)]"
              style={{ background: 'var(--cardinal)', boxShadow: '3px 3px 0 var(--black)' }}
            >
              CONTINUE
            </button>
          </div>
          ...
          /* ── Email + Password Form ── */
          <>
            <h2 className="font-display text-2xl tracking-wider mb-1" style={{ color: 'var(--black)' }}>
              {displayTitle}
            </h2>
            ...
            <form onSubmit={handleSubmit}>
              ...
              <div className="text-[10px] mb-3 font-display tracking-wider" style={{ color: 'var(--mid)' }}>
              ...
              {error && (
                <div
                  ...
                  style={{ borderColor: 'var(--cardinal)', color: 'var(--cardinal)', background: 'color-mix(in srgb, var(--cardinal) 5%, white)' }}
                >
                  {error}
                </div>
              )}
            ...
            <p className="text-xs text-center mt-4" style={{ color: 'var(--mid)' }}>
              {mode === 'signin' ? "Don't have an account? " : 'Already have an account? '}
              <button onClick={switchMode} className="underline" style={{ color: 'var(--cardinal)' }}>
