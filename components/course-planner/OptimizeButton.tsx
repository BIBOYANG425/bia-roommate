...
export default function OptimizeButton() {
  ...
  const handleOptimize = useCallback(async () => {
    if (active.selectedSections.length === 0) {
      dispatch({ type: 'SET_ERROR', error: 'ADD COURSES FIRST, THEN OPTIMIZE' })
      return
    }
    ...
    const courseIds = [...new Set(active.selectedSections.map((s) => s.courseId))]
    ...
    try {
      ...
      if (result.sections.length > 0) {
        ...
        dispatch({ type: 'SET_ERROR', error: 'NO VALID SCHEDULE FOUND — TRY DIFFERENT COURSES' })
      }
    } catch {
  ...
  return (
    <div>
      <button
        ...
        style={{
          opacity: state.isOptimizing || active.selectedSections.length === 0 ? 0.6 : 1,
        }}
      >
      ...
      {state.isOptimizing && (
        <div className="mt-2 h-2 border-[2px] border-[var(--black)]" style={{ background: 'var(--cream)' }}>
          <div
