# USCBIA GTM Implementation Plan

> **For the executor:** This is an operational marketing plan, not a code plan. Each task is a concrete deliverable with a deadline, owner slot, and done criteria. Work through tasks sequentially within each phase. Tasks across phases can overlap where noted.

**Goal:** Launch USCBIA's community-led GTM strategy to reach 500+ questionnaire completions and 200+ successful matches for the Fall 2026 USC admissions cycle.

**Approach:** Six parallel workstreams executed in four phases (Foundation → Seed → Peak Push → Harvest), March through September 2026.

**Key constraint:** Lean budget (¥6,500–12,500 / ~$900–1,700 USD). Community-led and content-first. Paid is optional amplification only.

---

## Phase 1: Foundation (March 27 – April 15)

### Task 1: Set Up WeChat Official Account (公众号)

**Owner:** \_\_\_
**Deadline:** April 1

**Steps:**

1. Register a WeChat Official Account (subscription type) under the org name "BIA新生找室友"
2. Set up profile: logo, bio using core message ("别让室友变室敌"), link to uscbia.com
3. Configure auto-reply for new followers: welcome message + questionnaire link
4. Set up menu bar: "开始匹配" → questionnaire link, "关于BIA" → about page, "联系我们" → contact

**Done when:** Account is live, auto-reply works, menu links resolve correctly.

---

### Task 2: Set Up Xiaohongshu Account

**Owner:** \_\_\_
**Deadline:** April 1

**Steps:**

1. Create Xiaohongshu account: "BIA找室友 | USC"
2. Profile bio: core message + "点击主页链接开始匹配"
3. Add uscbia.com link to profile
4. Follow and engage with 50+ accounts in #USC留学 #找室友 #留学生活 hashtags to seed the algorithm

**Done when:** Account is live, profile complete, initial follows done.

---

### Task 3: Create Brand Asset Kit

**Owner:** \_\_\_
**Deadline:** April 5

**Deliverables:**

1. **Brand color palette + typography guide** — consistent across WeChat, Xiaohongshu, and web
2. **Xiaohongshu carousel template** (Canva) — pastel aesthetic, BIA branding, 6-10 slide format
3. **WeChat article header template** — banner image with BIA branding
4. **WeChat group share card** — thumbnail + headline optimized for group chat previews
5. **Ambassador recruitment flyer** — one-pager explaining the program + benefits

**Done when:** All 5 assets are created and saved to a shared folder (Google Drive or Tencent Docs).

---

### Task 4: Build Partnership Outreach List

**Owner:** \_\_\_
**Deadline:** April 5

**Steps:**

1. Identify and list contact info for:
   - USC CSSA current board members (WeChat or email)
   - 3-5 留学中介 that send students to USC (e.g., 新东方前途, 启德, 金吉列, local boutique agencies)
   - USC Housing Office international student liaison
   - Any existing USC Chinese student housing WeChat groups
2. Draft outreach message template for each partner type (see Partnership Strategy in design doc for value prop per partner)
3. Save to a tracking spreadsheet: Partner name, contact, status, follow-up date

**Done when:** Spreadsheet has 10+ contacts with drafted outreach messages.

---

### Task 5: Draft First 4 Content Pieces

**Owner:** \_\_\_
**Deadline:** April 10

**Pieces to draft (1 per pillar):**

1. **Pillar 1 (Roommate Matching):** Xiaohongshu carousel — "你是哪种室友类型？5个问题测出你的室友人格"
   - 8-slide carousel: intro → 5 questions → 4 personality types → CTA to take full quiz on BIA
   - Tone: fun, relatable, shareable

2. **Pillar 2 (USC Living Guide):** WeChat article — "2026 USC住房全攻略：宿舍vs校外，哪个适合你？"
   - 1000-1200 words, compare on-campus vs off-campus on: cost, convenience, social life, privacy
   - Include neighborhood map graphic
   - End with soft CTA: "已经决定校外？先用BIA找个合适的室友"

3. **Pillar 3 (Student Life):** Xiaohongshu short post — "来USC第一年我踩过的5个坑"
   - Written as a personal story from a current student (interview a real student or write as composite)
   - Hashtags: #USC留学 #留学生活 #南加大 #美国留学

4. **Pillar 4 (Social Proof):** WeChat group share text — success story template
   - Short testimonial format: "我和室友通过BIA匹配，现在住了一年了，生活习惯完全合拍！"
   - Include before (worried about random roommate) → after (matched and happy) arc

**Done when:** All 4 pieces are drafted, reviewed, and ready to publish.

---

### Task 6: Recruit Student Ambassadors (Round 1)

**Owner:** \_\_\_
**Deadline:** April 15

**Steps:**

1. Post ambassador recruitment in existing USC Chinese student groups: "BIA校园大使招募 — 帮新生找到完美室友"
2. Target: recruit 5 ambassadors from current USC Chinese students
3. Each ambassador commits to:
   - Join 2-3 新生 WeChat groups from their home city
   - Share BIA content 2x/week during May–August
   - Report back weekly on group reactions/questions
4. Incentive: ¥200 红包 per ambassador for the season + BIA merch (if available)
5. Create a private ambassador WeChat group for coordination

**Done when:** 5 ambassadors confirmed, private group created, expectations documented.

---

## Phase 2: Seed (April 15 – May 15)

### Task 7: Launch Partnership Outreach

**Owner:** \_\_\_
**Deadline:** April 20

**Steps:**

1. Send outreach messages to all contacts from Task 4
2. For CSSA: propose co-branded "新生找室友" post on their official account + mention in orientation packet
3. For 留学中介: offer a one-pager they can share with USC-bound clients, branded with their logo + BIA
4. For Housing Office: email introducing BIA as a free student resource, ask to be listed on their international student resources page
5. Follow up within 5 days if no response
6. Update tracking spreadsheet with responses

**Done when:** All outreach sent, at least 2 partnerships confirmed.

---

### Task 8: Publish Seed Content (Week 1-2)

**Owner:** \_\_\_
**Deadline:** April 30

**Publishing schedule:**

- **April 16:** Xiaohongshu — "你是哪种室友类型？" carousel (Task 5, Piece 1)
- **April 18:** WeChat 公众号 — "2026 USC住房全攻略" article (Task 5, Piece 2)
- **April 22:** Xiaohongshu — "来USC第一年我踩过的5个坑" (Task 5, Piece 3)
- **April 25:** WeChat groups — share success story text (Task 5, Piece 4)

**For each post:**

1. Publish on platform
2. Share in ambassador group for resharing
3. Track engagement: views, likes, saves, comments, click-throughs to uscbia.com
4. Respond to every comment within 24 hours

**Done when:** All 4 pieces published, engagement tracked in spreadsheet.

---

### Task 9: Infiltrate 新生 WeChat Groups

**Owner:** Ambassadors + core team
**Deadline:** May 1

**Steps:**

1. Identify USC 2026 admitted student WeChat groups — sources:
   - CSSA new student group
   - 留学中介 client groups
   - Xiaohongshu/Weibo posts sharing group QR codes
   - City-specific groups (北京, 上海, 深圳, 广州 etc.)
2. Join as many as possible (target: 10+ groups)
3. **Do NOT spam.** First 2 weeks: just be helpful. Answer housing questions, share USC tips.
4. After establishing presence: share BIA link naturally when someone asks about roommates
5. Track which groups have been joined in the spreadsheet

**Done when:** Present in 10+ groups, first organic BIA mentions posted.

---

### Task 10: Produce May Content Batch

**Owner:** \_\_\_
**Deadline:** May 10

**Produce 8 pieces for May publishing (2 per week):**

| #   | Platform          | Pillar       | Topic                                                          |
| --- | ----------------- | ------------ | -------------------------------------------------------------- |
| 1   | Xiaohongshu       | Matching     | "室友红旗🚩 这5种人千万别合租" carousel                        |
| 2   | WeChat 公众号     | Living Guide | "USC周边租房地图：University Park vs Koreatown vs DTLA"        |
| 3   | Xiaohongshu       | Matching     | "BIA匹配算法大揭秘 — 我们怎么帮你找到最合拍的室友" infographic |
| 4   | WeChat 公众号     | Student Life | Interview: "我的USC第一年" with a real current student         |
| 5   | Xiaohongshu       | Living Guide | "签lease前必看！美国租房避坑指南"                              |
| 6   | WeChat group text | Social Proof | First real BIA match testimonial (if available)                |
| 7   | Xiaohongshu       | Student Life | "USC留学必备App清单" carousel                                  |
| 8   | WeChat 公众号     | Matching     | "为什么随机分配室友是个坏主意 — 数据告诉你"                    |

**Done when:** All 8 pieces drafted, designed, and scheduled.

---

## Phase 3: Peak Push (May 15 – July 31)

### Task 11: Execute May–June Content Calendar

**Owner:** \_\_\_
**Deadline:** Ongoing, May 15 – June 30

**Weekly cadence:**

- **Monday:** Publish Xiaohongshu post
- **Wednesday:** Publish WeChat 公众号 article
- **Friday:** Share in WeChat groups (link to article or standalone text)

**Weekly ops:**

1. Publish per schedule
2. Share each post in ambassador group for amplification
3. Respond to all comments/DMs within 24 hours
4. Log metrics weekly: views, engagement, uscbia.com traffic, questionnaire completions

**Done when:** 8 weeks of consistent publishing completed with metrics logged.

---

### Task 12: Launch Share-to-Unlock Growth Mechanic

**Owner:** \_\_\_ (requires dev work)
**Deadline:** May 20

**Steps:**

1. On uscbia.com, after questionnaire completion, show partial compatibility report
2. Prompt: "分享给3位好友，解锁完整匹配报告" with WeChat share button
3. Track shares → completions conversion rate
4. If conversion is <5%, simplify to "share with 1 friend"

**Done when:** Share-to-unlock is live, tracking is working, first week data collected.

---

### Task 13: Launch Pair Reveal Notifications

**Owner:** \_\_\_ (requires dev work)
**Deadline:** May 25

**Steps:**

1. When two users are matched, send both a notification (WeChat template message or email)
2. Include: match name, top 3 compatibility highlights, link to connect
3. Add a "screenshot and share" prompt in the match reveal page — make it visually attractive for sharing

**Done when:** Match notifications are sending, at least 5 pairs have connected.

---

### Task 14: Optional — Xiaohongshu KOC Seeding

**Owner:** \_\_\_
**Deadline:** June 1
**Budget:** ¥2,000–5,000

**Steps:**

1. Identify 10-20 Xiaohongshu creators in 留学 niche (1K–50K followers)
2. DM with pitch: "We'll give you early access to BIA + ¥100-200, you post an honest review"
3. Provide each creator with: their compatibility type result, talking points, BIA link
4. Track which creators post and resulting traffic

**Done when:** 10+ KOC posts live, traffic from Xiaohongshu tracked.

---

### Task 15: Optional — WeChat Moments Ads

**Owner:** \_\_\_
**Deadline:** June 15
**Budget:** ¥3,000–5,000

**Steps:**

1. Create ad creative: lead with "你是哪种室友类型？" quiz hook
2. Target: 18-22 year olds in Beijing, Shanghai, Shenzhen, Guangzhou with 留学 interest tags
3. Run for 2 weeks (June 15–30) during peak housing-hunting season
4. A/B test two creatives, cut the loser after 3 days
5. Track: impressions → clicks → questionnaire completions → cost per completion

**Done when:** Ad campaign completed, results logged, CPA calculated.

---

### Task 16: Produce July Content Batch + Urgency Push

**Owner:** \_\_\_
**Deadline:** July 5

**July content shifts to urgency messaging:**

| #   | Platform          | Topic                                                       |
| --- | ----------------- | ----------------------------------------------------------- |
| 1   | Xiaohongshu       | "还没找到室友？离开学只剩6周了" urgency carousel            |
| 2   | WeChat 公众号     | "BIA已经匹配了X对室友 — 你的完美室友还在等你" stats article |
| 3   | Xiaohongshu       | "8月租房checklist：搬去LA前必须做完这10件事"                |
| 4   | WeChat group text | "最后一波匹配！8月15日前完成问卷"                           |
| 5   | Xiaohongshu       | User story: real matched pair, photo + quote                |
| 6   | WeChat 公众号     | "室友合租协议模板 — 住在一起前先谈好这些"                   |

**Done when:** All 6 pieces published by July 31.

---

## Phase 4: Harvest (August – September)

### Task 17: Collect Testimonials & Success Stories

**Owner:** \_\_\_
**Deadline:** September 15

**Steps:**

1. DM all successfully matched pairs: "你们住在一起了吗？分享你们的故事！"
2. Collect: photo together (if willing), 2-3 sentence quote, rating of BIA experience
3. Target: 10+ testimonials with photos
4. Get permission to use on Xiaohongshu, WeChat, and uscbia.com
5. Save all testimonials to a content library for next cycle

**Done when:** 10+ testimonials collected with usage permission.

---

### Task 18: End-of-Cycle Metrics Report

**Owner:** \_\_\_
**Deadline:** September 30

**Compile final metrics:**

1. Questionnaire completions (target: 500+)
2. Successful matches — both parties connected (target: 200+)
3. WeChat 公众号 followers (target: 1,000+)
4. Xiaohongshu posts — own + UGC (target: 50+)
5. Partner channels activated (target: 5+)
6. Content performance: top 3 posts by engagement, top 3 by conversion
7. Channel breakdown: which channel drove the most questionnaire completions
8. Budget spent vs. plan
9. Ambassador performance: who drove the most shares/signups

**Deliverable:** One-page summary + detailed spreadsheet.

**Done when:** Report complete and shared with team.

---

### Task 19: Set Up Next Cycle Flywheel

**Owner:** \_\_\_
**Deadline:** October 15

**Steps:**

1. Publish "BIA 2026年度报告" on WeChat 公众号 — X matches, X% compatibility satisfaction, best stories
2. Identify top 3 ambassadors → invite to return as leads next cycle
3. Archive all content templates, outreach templates, and tracking spreadsheets for reuse
4. Document what worked and what didn't — save as internal retro doc
5. Begin relationship-building for Fall 2027 cycle partnerships (CSSA board transitions happen in spring)

**Done when:** Retro doc written, top ambassadors invited back, templates archived.

---

## Execution Tracker

| Task                          | Phase      | Deadline | Owner  | Status |
| ----------------------------- | ---------- | -------- | ------ | ------ |
| 1. WeChat Official Account    | Foundation | Apr 1    | \_\_\_ | ⬜     |
| 2. Xiaohongshu Account        | Foundation | Apr 1    | \_\_\_ | ⬜     |
| 3. Brand Asset Kit            | Foundation | Apr 5    | \_\_\_ | ⬜     |
| 4. Partnership Outreach List  | Foundation | Apr 5    | \_\_\_ | ⬜     |
| 5. First 4 Content Pieces     | Foundation | Apr 10   | \_\_\_ | ⬜     |
| 6. Recruit Ambassadors        | Foundation | Apr 15   | \_\_\_ | ⬜     |
| 7. Launch Outreach            | Seed       | Apr 20   | \_\_\_ | ⬜     |
| 8. Publish Seed Content       | Seed       | Apr 30   | \_\_\_ | ⬜     |
| 9. Infiltrate 新生 Groups     | Seed       | May 1    | \_\_\_ | ⬜     |
| 10. May Content Batch         | Seed       | May 10   | \_\_\_ | ⬜     |
| 11. May–June Content Calendar | Peak       | Jun 30   | \_\_\_ | ⬜     |
| 12. Share-to-Unlock (dev)     | Peak       | May 20   | \_\_\_ | ⬜     |
| 13. Pair Reveal Notifs (dev)  | Peak       | May 25   | \_\_\_ | ⬜     |
| 14. KOC Seeding (optional)    | Peak       | Jun 1    | \_\_\_ | ⬜     |
| 15. WeChat Ads (optional)     | Peak       | Jun 15   | \_\_\_ | ⬜     |
| 16. July Urgency Content      | Peak       | Jul 5    | \_\_\_ | ⬜     |
| 17. Collect Testimonials      | Harvest    | Sep 15   | \_\_\_ | ⬜     |
| 18. Metrics Report            | Harvest    | Sep 30   | \_\_\_ | ⬜     |
| 19. Next Cycle Flywheel       | Harvest    | Oct 15   | \_\_\_ | ⬜     |
