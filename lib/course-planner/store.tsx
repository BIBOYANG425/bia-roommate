...
function updateActiveSchedule(state: PlannerState, updater: (s: Schedule) => Schedule): PlannerState {
  return {
    ...state,
...
function plannerReducer(state: PlannerState, action: PlannerAction): PlannerState {
  switch (action.type) {
    ...
    case 'SET_EXPANDED_COURSE':
      return { ...state, expandedCourse: action.course, isLoadingCourse: false }
    ...
    case 'ADD_SECTION': {
      return updateActiveSchedule(state, (sched) => {
        ...
        return { ...sched, selectedSections: sections, totalUnits: calcUnits(sections) }
      })
    }
    ...
    case 'REMOVE_SECTION': {
      return updateActiveSchedule(state, (sched) => {
        const sections = sched.selectedSections.filter(
          (s) => !(s.courseId === action.courseId && s.section.id === action.sectionId)
        )
        return { ...sched, selectedSections: sections, totalUnits: calcUnits(sections) }
      })
    }
    ...
    case 'REMOVE_COURSE': {
      return updateActiveSchedule(state, (sched) => {
        const sections = sched.selectedSections.filter((s) => s.courseId !== action.courseId)
        return { ...sched, selectedSections: sections, totalUnits: calcUnits(sections) }
      })
    }
    ...
    case 'CACHE_RMP':
      return { ...state, rmpCache: { ...state.rmpCache, [action.key]: action.rating } }
    ...
    case 'SET_OPTIMIZING':
      return {
        ...
        optimizeProgress: action.progress ?? (action.isOptimizing ? 0 : state.optimizeProgress),
      }
