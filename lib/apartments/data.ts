// LA apartments dataset (single source of truth) + presentation lookups.
// Each apartment object carries its own valueScore / luxuryScore / funTags /
// rankBoost — adding a new apartment is a single object edit. STATIC_RANKING is
// derived from the array (rankBoost + summed reddit-seed scores, sorted desc).
// Consumed by app/apartments/page.tsx and app/apartments/_components/*.

import type { ProductLanguage } from "@/components/ProductShell";

export type Neighborhood = "South Park" | "DTLA" | "Koreatown";
...
export interface Apartment {
  ...
  valueScore: 1 | 2 | 3 | 4; // amenities-vs-price rating
  luxuryScore: 1 | 2 | 3 | 4 | 5;
  funTags: string[];
  rankBoost?: number; // editorial nudge added to the口碑 ranking score (default 0)
}
...
export const VALUE_LABELS: Record<1 | 2 | 3 | 4, { zh: string; en: string; color: string }> = {
...
export const LUXURY_STARS = ["", "★☆☆☆☆", "★★☆☆☆", "★★★☆☆", "★★★★☆", "★★★★★"] as const;
...
// Derived口碑 ranking: rankBoost (default 0) + summed reddit-seed scores, desc.
export const STATIC_RANKING: RankingEntry[] = [...APARTMENTS]
  .map((apt) => ({
    id: apt.id,
    name: apt.name,
    accentColor: apt.accentColor,
    score: (apt.rankBoost ?? 0) + apt.redditSeeds.reduce((s, r) => s + r.score, 0),
  }))
  .sort((a, b) => b.score - a.score);

// Filter dropdown options (referenced by the apartments page UI).
...
export const NEIGHBORHOOD_OPTIONS: { label: Record<ProductLanguage, string>; value: Neighborhood | "" }[] = [
