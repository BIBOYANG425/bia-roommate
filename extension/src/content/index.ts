...
function removeRmpBadges() {
  const badges = document.querySelectorAll(
    ".bia-rmp-badge, .bia-rmp-badge-loading",
  );
...
function removeSeatBadges() {
  const badges = document.querySelectorAll(
    ".bia-seat-badge, .bia-seat-summary",
  );
  for (const b of badges) b.remove();
  ...
  for (const el of headerProcessed)
    el.removeAttribute("data-bia-seats-summary");
}
...
function init() {
  ...
  function waitForContent() {
    ...
    if (!hasContent) {
      console.log(
        "[BIA] No course content found after 10s, setting up observer only",
      );
    }
...
const isLoginPage =
  document.querySelector('#loginForm, .shib-login, [action*="login"]') !== null;
if (isLoginPage) {
