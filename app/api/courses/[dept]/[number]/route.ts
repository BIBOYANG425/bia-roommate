...
function formatPrereqs(prereqs: any[]): string {
  ...
  return prereqs
    .map((group: any) => {
      const options = (group.courseOptions || []).map(
        (o: any) => o.courseSpace || o.courseHyphen || "",
      );
...
export async function GET(
  ...
  try {
    ...
    if (data.majorRestrictions)
      restrictionsList.push(`Major: ${data.majorRestrictions}`);
    if (data.schoolRestrictions)
      restrictionsList.push(`School: ${data.schoolRestrictions}`);
    const restrictions =
    ...
    const course = {
      ...
      number:
        (data.scheduledCourseCode?.number || number) +
        (data.scheduledCourseCode?.suffix || ""),
