...
import { relativeTime, schoolAccent, schoolTagClass, getLastChar } from '@/lib/utils'
    ...
    <div className="border-[3px] border-[var(--black)]" style={{ background: 'var(--cream)' }}>
      ...
      >
        ...
        <div className="flex items-center gap-2">
          {count > 0 && (
            <span className="px-2 py-0.5 text-[10px] border-[2px] border-[var(--black)]"
              style={{ background: 'var(--gold)' }}>
              {count}
            </span>
          )}
          <span className="text-xs" style={{ transform: open ? 'rotate(180deg)' : 'none', transition: 'transform 0.2s', display: 'inline-block' }}>
            &#9660;
          </span>
        </div>
      </button>
...
function StatBox({ label, count }: { label: string; count: number }) {
  return (
    ...
    >
      <div className="font-display text-2xl tracking-wider" style={{ color: 'var(--black)' }}>
        {count}
      </div>
      <div className="text-[10px] tracking-wider mt-1" style={{ color: 'var(--mid)' }}>
        {label}
      </div>
    </div>
  )
    ...
    <div className="max-w-2xl mx-auto px-4 py-8 space-y-4">
      ...
      {loading ? (
        ...
        >
          ...
          <div className="flex items-start gap-4">
            ...
            <div className="min-w-0 flex-1 pr-8">
              ...
              <p className="text-xs mt-1" style={{ color: 'var(--mid)' }}>
                {[profile.school, profile.year, profile.major, profile.gender].filter(Boolean).join(' / ')}
              </p>
            </div>
          </div>
          ...
          {/* Bio */}
          {profile.bio && (
            <p className="text-sm mt-3 leading-relaxed" style={{ color: 'var(--black)' }}>
              {profile.bio}
            </p>
          )}
        ...
        <div
          ...
          style={{ background: 'var(--cream)' }}
        >
          <h2 className="font-display text-xl tracking-wider mb-2" style={{ color: 'var(--black)' }}>
                  ...
                  >
                    <div>
                      ...
                      <p className="text-[11px] mt-1" style={{ color: 'var(--mid)' }}>
                        {s.semester} &mdash; {Array.isArray(s.courses) ? s.courses.map((c) => c.code || c).join(', ') : ''}
                      </p>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mt-3">
                {likedProfiles.map((p) => (
                  <div key={p.id} className="relative">
                    <MiniProfileCard profile={p} onClick={() => router.push(`/?profile=${p.id}`)} />
                    <button
                      onClick={(e) => { e.stopPropagation(); handleUnlike(p.id) }}
                  ...
                  >
                    <div className="flex items-start justify-between gap-3">
                      <div className="min-w-0 flex-1">
                        {c.profile && (
                          <p className="font-display text-xs tracking-wider mb-1" style={{ color: 'var(--mid)' }}>
                            ON{' '}
...
/* ── Mini Profile Card ── */
function MiniProfileCard({ profile, onClick }: { profile: RoommateProfile; onClick: () => void }) {
    ...
    >
      ...
      <div className="min-w-0 flex-1 pr-6">
        ...
        <p className="text-[10px] truncate" style={{ color: 'var(--mid)' }}>
          {[profile.school, profile.major, profile.year].filter(Boolean).join(' / ')}
        </p>
...
function EmptyState({ message }: { message: string }) {
  return (
    <div className="py-6 text-center">
      <p className="font-display text-sm tracking-wider" style={{ color: 'var(--mid)' }}>
        {message}
      </p>
    </div>
  )
